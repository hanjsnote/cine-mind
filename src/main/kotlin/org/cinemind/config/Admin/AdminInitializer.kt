...
class AdminInitializer (
    ...
    // 이메일은 환경 변수 `ADMIN_EMAIL`에서 주입받고, 없으면 `admin@cinemind.org`를 기본값으로 사용
    @Value("\${ADMIN_EMAIL:admin@cinemind.org}")
    private val adminEmail: String,

    // 비밀번호는 환경 변수 `ADMIN_PASSWORD`에서 주입받고, 없으면 `default-admin1234`를 기본값으로 사용
    @Value("\${ADMIN_PASSWORD:default-admin1234}")
